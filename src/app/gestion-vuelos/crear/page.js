...
export default function CrearVueloPage() {
  ...
  async function onSubmit(e) {
    ...
    try {
      ...
      const res = await fetch("/api/dorado/vuelos/crear", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          codvuelo: form.codvuelo.trim(),
      ...
      if (!res.ok) throw new Error(data?.message || "No se pudo registrar el vuelo");
      setSuccess("Vuelo registrado correctamente");
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 to-slate-950 p-4 md:p-8">
      <div className="mx-auto max-w-5xl bg-slate-900/60 border border-white/10 rounded-3xl shadow-2xl overflow-hidden">
        ...
        <div className="px-6 pt-6">
          <h2 className="text-slate-300 text-lg font-medium">Crear Vuelo - Formulario de Registro</h2>
        </div>
